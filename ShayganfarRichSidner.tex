...
dimensions are only derived for the relevant events. We present our first
algorithm (Algorithm \ref{alg:relevance}) for determining the relevance of the
given event with respect to the shared goal. Based on this algorithm, an event
is \textit{irrelevant} if there is no connection between the event and the
active shared goal. However, if there is a connection, the relevance of the
event will depend on the significance of the event with respect to the current
collaboration status. The significance of an event is determined based on the
utility of the event as it is also presented in
\cite{gratch:domain-independent,marsella:ema-process-model}. We believe
although the utility of the event represents the sigificance of the event, the
other collaborator's (human's) expressed emotion also plays a role by
influencing the significance of a utility through a threshold. The algorithm
...
the current mental graph ($g_{t}$). After perceiving an event, the structure of
the event will include a belief about that event which contains all the required
information such as the belief attributes mentioned in Section
\ref{sec:mental-states@cref}. Continuing in line 3, the $g_{t}$ represents the
shared goal (in the mental graph) at time (turn) $t$ within the shared plan. In
this line, \textit{F{\fontsize{8}{8}\selectfont IND}P{\fontsize{8}{8}\selectfont
ATH}} returns the shortest path between the corresponding nodes of
$\mathit{b}_{\varepsilon_t}$ and $g_{t}$ in the mental graph. The
...
the human collaborator. In the algorithm, the \textit{G{\fontsize{8}{8}\selectfont
...
attributes associated with the belief about that event, i.e.,
...
value which consequently makes the robot see an event with even a slightly
positive utility as a relevant one. Finally, if
$\mathcal{U}_{\mathit{b}_{\varepsilon_t}}$ is greater than or equal to
$\tau_{e}$ the event will be considered \textit{relevant}; otherwise the event
will be considered \textit{irrelevant} even though there is a path between
