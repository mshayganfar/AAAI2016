...
environment. In a collaborative environment, tasks are delegated to the
...
\ref{alg:controllability}, $\alpha$ denotes the value of agency with regard to
...
responsible over the total number of contributing goal to
$\mathit{g}_{\varepsilon_t}$ (see Algorithm \ref{alg:agency}). If the associated
goal with the current event corresponds to a nonprimitive task, the algorithm
checks the responsible agent for each primitive task that contributes to the
nonprimitive one and returns a value of which $(0 \leq \alpha \leq 1)$. However,
if the associated goal to the current event corresponds to a primitive task the
value of $\alpha$ would be 0 or 1.
...
The quality or state of being self-governing. Self-directing freedom or
