...
the current mental graph ($g_{t}$). After perception of an event, the structure
...
mental graph) at time (turn) $t$ within the shared plan. Next, \textit{FINDPATH}
...
\textit{FINDPATH} function uses the Dijkstra algorithm to find the shortest
path. Now, if there is no path between the belief about the current event and
the active shared goal, the algorithm finds the event \textit{irrelevent} to
the current shared goal and terminates. However, if there is a path between the
$\mathit{b}_{\varepsilon_t}$ and $g_{t}$, the
\textit{G{\fontsize{8}{8}\selectfont ET}E{\fontsize{8}{8}\selectfont
VENT}U{\fontsize{8}{8}\selectfont TILITY}} function computes the utility of the
belief about the event $\mathcal{U}_{\mathit{b}_{\varepsilon_t}}$, $(0 \leq
\mathcal{U}_{\mathit{b}_{\varepsilon_t}} \leq 1)$ based on the values of the
attributes associated to the belief.
