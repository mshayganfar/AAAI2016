...
biased towards those events facilitating the progress in collaboration. An event
...
and consequently the robot's mental state consistent to the collaboration
status. Therefore, it causes the robot to dismiss events causing
inconsistencies in robot's collaborative behavior. Moreover, desirability is
also crucial from the collaboration's point of view. A collaborative robot needs
to know whether the robot and the other collaborator's actions, utterances, and
emotional expressions are desirable in terms of their consistence with the
status of the current shared goal. In other words, the collaboration mechanism
uses the appraisal process of desirability to coordinate what the self or the
other do, say, and express during collaboration. And correspondingly, in general
the appraisal mechanism and more specifically the desirability process use the
collaboration structure to obtain their required information to process.
...
underiable by the robot. The algorithm continues in case of an unknown status of
the current shared goal, and checks whether the precondition(s) of the
associated goal to the current event are satisfied (lines 17 to 21).
Intuitively, the robot prefers the satisfied perconditions and interprets the
event as desirable while unsatisfied perconditions are undesirable for the
...
ambiguity of the associted goal to the current event (line 15). An ambiguous
...
to the current event are unknown, the robot checks whether this goal contributes
to the current shared goal (lines 22 to 25). As a result a contributing goal will
obtain a neutral desirability in compare to a noncontributing goal which will be
