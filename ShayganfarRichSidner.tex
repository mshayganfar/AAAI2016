...
All of the algorithms use mental graph which is a directed acyclic graph
cunstructed based on the mental states of the robot at each turn during the
...
action, utterance, and emotional expression.

The very first three algorithms, i.e., \textit{Relevance} (Algorithm
\ref{alg:relevance}), \textit{Desirability} (Algorithm \ref{alg:desirability}),
and \textit{Expectedness} (Algorithm \ref{alg:expectedness}) are the ones we
found more important between the appraisal variables in collaboration context.
There are other appraisal variables introduced in psychological
\cite{scherer:appraisal-processes} and computational litrature
\cite{marsella:computational-models} which we do not provide algorithms for them
in this paper. The concept behind these variables are either too simple, e.g.,
\textit{Perspective} (i.e., the view point that an event judged), or it requires
a particular approach which is not our concern in this paper, e.g.,
\textit{Likelihood} (i.e., how probable is the outcome of an event)

\subsection{Relevance}

We present our first algorithm for determining the relevance of the given
external event with respect to the status of the shared goal. The algorithm
starts by taking the belief about the current event
($\mathit{b}_{\varepsilon_t}$), and extracting the shared goal associated with
the current mental graph ($g_{t}$). After perceiving an event, the structure
of the event will include a belief about that event which contains all the
required information such as the belief attributes mentioned in Section
\cref{sec:mental-states@cref}. The event will also include a corresponding goal
irrespective of the event type (i.e., action, utterance, or emotional
expression) which will be used in our following algorithms (see line 19 in
Algorithm \ref{alg:desirability}). This event structure holds in all of our
algorithms. Continuing in line 3, the $g_{t}$ represents the shared goal (in
mental graph) at time (turn) $t$ within the shared plan. In this line,
\textit{F{\fontsize{8}{8}\selectfont IND}P{\fontsize{8}{8}\selectfont ATH}}
receives the belief about the event and the current shared goal as input, and
returns the shortest path between these two nodes in the mental graph. The
\textit{F{\fontsize{8}{8}\selectfont IND}P{\fontsize{8}{8}\selectfont ATH}}
function uses the Dijkstra algorithm to find the shortest path. Now, if there is
no path between the belief about the current event and the active shared goal,
the algorithm finds the event \textit{irrelevent} to the current shared goal,
and terminates. However, if there is a path between $\mathit{b}_{\varepsilon_t}$
and $g_{t}$, we need a way to determine whether the event is relevant to the
current collaboration status. Thus, we compute the utility of an event and make
our decision with respect to the human's emotional state. In other words, the
utility of a belief about an event is influenced by the perceived emotion of the
human collaborator. In the algorithm, the \textit{G{\fontsize{8}{8}\selectfont
ET}E{\fontsize{8}{8}\selectfont VENT}U{\fontsize{8}{8}\selectfont TILITY}}
function (see line 8 in Algorithm \ref{alg:relevance}) computes the utility of
the event $\mathcal{U}_{\mathit{b}_{\varepsilon_t}}(0 \leq
\mathcal{U}_{\mathit{b}_{\varepsilon_t}} \leq 1)$ based on the values of the
attributes associated to the belief about that event, i.e.,
$\mathit{b}_{\varepsilon_t}$. And, the human's emotion influences the decision
about the utility of the event in form of a threshold value ($\tau_{e}$).
For instance, a positive expressed emotion of the human reduces the threshold
value which consequently makes the robot to see an event with even a slightly
positive utility as a relevant one. Finally, if
$\mathcal{U}_{\mathit{b}_{\varepsilon_t}}$ is greater than or equal to
$\tau_{e}$ the event will be considered as \textit{relevant}, otherwise the
event will be considered as \textit{irrelevant} even though that there is a path
between $\mathit{b}_{\varepsilon_t}$ and $g_{t}$.

\begin{algorithm}
	\caption{(Relevance)}
	\label{alg:relevance}
	\begin{algorithmic}[1]
		\Function{IsEventRelevant}{Graph $\mathcal{G}_{t}$, \textit{Event}
		$\varepsilon_t$}
			\Statex
			...
			\Else
				\State $\mathcal{U}_{\mathit{b}_{\varepsilon_t}} \gets
				\Call{getEventUtility}{\mathit{b}_{\varepsilon_t}, \mathit{g}_{t}}$
				\State $\tau_{e} \gets \Call{getEmotionalThreshold}{ }$
				\If {$( \mathcal{U}_{\mathit{b}_{\varepsilon_t}} \geq \tau_{e} )$}
...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		\Function{IsEventDesirable}{Graph $\mathcal{G}_{t}$, \textit{Event}
		$\varepsilon_t$}
			\Statex
			...
			INPROGRESS})}
				...
				\ElsIf {(\textit{curGoalStatus()} = {\fontsize{8}{8}\selectfont UNKNOWN})}
					\Statex
					\If {(\textit{taskPreStatus()} = {\fontsize{8}{8}\selectfont SATISFIED})}
						\State \Return {{\fontsize{8}{8}\selectfont LOW-DESIRABLE}}
					\ElsIf {(\textit{taskPreStatus()} =	{\fontsize{8}{8}\selectfont UNSATISFIED})} 
						\State \Return {{\fontsize{8}{8}\selectfont LOW-UNDESIRABLE}}
					\ElsIf {(\textit{taskPreStatus()} = {\fontsize{8}{8}\selectfont UNKNOWN})}
						\Statex
						...
						\EndIf
						\Statex
						\If {(\textit{doesContribute}$(\mathit{g}_{\varepsilon_t}, \mathit{g}_{t}))$}
...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		\Function{IsEventExpected}{Graph $\mathcal{G}_{t}$, \textit{Event}
		$\varepsilon_t$}
			\Statex
			\State $\mathit{g}_{t} \gets \Call{extractGoal}{\mathcal{G}_{t}}$
			\State $\mathit{g}_{\varepsilon_t} \gets \Call{getGoal}{\varepsilon_t}$
			...
			\Else
				\If {$(\neg \Call{IsAchieved}{\mathit{g}_{t}})$}
					\State \Return {\fontsize{8}{8}\selectfont UNEXPECTED}
				\Else
					\State $\mathcal{P}_{t} \gets \Call{findPath}{\mathit{b}_{\varepsilon_t},
					\mathit{g}_{t}}$
...
\begin{algorithm}
	...
	\label{array-sum}
	\begin{algorithmic}[1]
		\Function{GetSucPredecessorsRatio}{\textit{Event} $\varepsilon_t$}
			\Statex
			\State $\mathit{g}_{\varepsilon_t} \gets \Call{getGoal}{\varepsilon_t}$
			\Statex
			\If {$(\mathit{g}_{\varepsilon_t} =$ {\fontsize{8}{8}\selectfont
...
\begin{algorithm}
	...
	\label{array-sum}
	\begin{algorithmic}[1]
		\Function{GetAvailableInputRatio}{\textit{Event} $\varepsilon_t$}
			\Statex
			\State $\mathit{g}_{\varepsilon_t} \gets \Call{getGoal}{\varepsilon_t}$
			\Statex
			\If {$(\mathit{g}_{\varepsilon_t} =$ {\fontsize{8}{8}\selectfont
...
\begin{algorithm}
	...
	\label{array-sum}
	\begin{algorithmic}[1]
		\Function{GetAgencyRatio}{\textit{Event} $\varepsilon_t$}
			\Statex
			\State $\mathit{g}_{\varepsilon_t} \gets \Call{getGoal}{\varepsilon_t}$
			\Statex
			\If {$(\mathit{g}_{\varepsilon_t} =$ {\fontsize{8}{8}\selectfont
...
\begin{algorithm}
	...
	\label{array-sum}
	\begin{algorithmic}[1]
		\Function{GetAutonomyRatio}{\textit{Event} $\varepsilon_t$}
			\Statex
			\State $\mathit{b}_{\varepsilon_t} \gets \Call{getBelief}{\varepsilon_t}$
			\State $\mathit{g}_{\varepsilon_t} \gets \Call{getGoal}{\varepsilon_t}$
			\Statex
