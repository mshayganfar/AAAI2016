...
associated to the event and the belief about the event. The goals can be
associated to any type of event including action, utterance, or emotional
expression.
...
paper. We believe most of these variables can be added straightforwardly later
to our appraisal mechanism, e.g., \textit{Perspective} (i.e., the view point from
which an event is judged).
...
three-dimensional space of the somatic markers associated to the other's
...
meanwhile the controllability uses the information from the collaboration
structure, e.g., successful predecessors of a goal.
...
over the occurring events. This sense of subjective control is based on one's
...
this stage of the project. We will use these weights after investigating more
about the influence of these factors, and implementing other mechanisms in the
overall architecture. After computing the value of the utility, we compare
this value to an emotional threshold similar to what we discussed in Algorithm
\ref{alg:relevance}. This comparison leads to our decision about the
...
act independently of each other. Hence, they need to have some internal motives
...
and the recognized goal associated with that event, we exploit the motive within
